...
@Autonomous
public class VisionAzulLargo extends CommandOpMode {
    double cX = 0;
    double cY = 0;
    double width = 0;
        ...
        while(!isStarted()){
            ...
            if (getDistance(width) < 100) {
                if (cX < 270 && cX > 1) {
                    ...
                    CommandScheduler.getInstance().schedule(new AzulIzqLrg(drive, intake, elevador));
                } else if (cX > 270 && cX < 450) {
                    telemetry.addLine("En medio");
                    CommandScheduler.getInstance().schedule(new AzulMidLrg(drive, intake, elevador));
                }
            }
            else{
                telemetry.addLine("Lado derecho");
                CommandScheduler.getInstance().schedule(new AzulDerLrg(drive, intake, elevador));
            }
